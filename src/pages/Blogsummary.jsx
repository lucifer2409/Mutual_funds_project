import React from 'react'
import Navbar from '../Navbar'
import logo from '../assets/Logo.svg'
import facebook from '../assets/facebook.svg'
import twitter from '../assets/twitter.svg'
import linkedin from '../assets/linkedin.svg'
import insta from '../assets/insta.svg'
import shareicon from '../assets/shareicon.svg'
import youtube from '../assets/youtube.svg'
import instablack from '../assets/instablack.svg'
import facebookblack from '../assets/facebookblack.svg'
import xblack from '../assets/xblack.svg'
import confusedman from '../assets/confusedman.png'
import linkedinblack from '../assets/linkedinblack.svg'
import blogimage from '../assets/blogimage.png'
import rectangleshape from '../assets/rectangleshape.svg'
import image1 from '../assets/image1.svg'
import image2 from '../assets/image2.svg'
import image3 from '../assets/image3.svg'
import image4 from '../assets/image4.svg'
import image5 from '../assets/image5.svg'


const Blogsummary = () => {
    return (
        <>
            <Navbar />
            <div id='topsection2'>
                <div id='insidetopsection2ofblogsummary'>
                    <div id='topsectionofinsidetopsection2ofblogsummary'>
                        <div id='filterbuttonsforblogsummary'>
                            <button className='filterbuttonforblogsummary'>
                                Mutual Fund
                            </button>
                            <button className='filterbuttonforblogsummary'>
                                SIP Lumsum
                            </button>
                        </div>
                        <span id='blogsummaryheading'>
                            Money Mind Games: Why You Splurge and Save, and How Mutual Funds Can Help!
                        </span>
                    </div>
                    <div id='bottomsectionofinsidetopsection2ofblogsummary'>
                        <div id='leftsectionofbottomsectionofinsidetopsection2ofblogsummary'>
                            <span className='leftsectiontext'>Jul 9, 2024</span>
                            <svg xmlns="http://www.w3.org/2000/svg" width="2" height="16" viewBox="0 0 2 16" fill="none">
                                <path d="M1 0V16" stroke="#E2E2E2" />
                            </svg>
                            <span className='leftsectiontext'>5 min read</span>
                        </div>
                        <div id='rightsectionofbottomsectionofinsidetopsection2ofblogsummary'>
                            <img src={facebookblack} alt="" className='blackicons' />
                            <img src={xblack} alt="" className='blackicons' />
                            <img src={linkedinblack} alt="" className='blackicons' />
                            <img src={instablack} alt="" className='blackicons' />
                        </div>
                    </div>
                </div>
            </div>
            <div id='middlesectionofblogsummary'>
                <div id='leftsectionofmiddlesectionofblogsummary'>
                    <div id='topsectionforleftsectionofmiddlesectionofblogsummary'>
                        <div id='topoftopsectionforleftsectionofmiddlesectionofblogsummary'>
                            <img src={rectangleshape} alt="" id='rectangleshape' />
                            <span id='summaryheading'>
                                Summary
                            </span>
                        </div>
                        <span id='summarytext'>
                            Ever wondered why we tend to splurge our tax refunds but scrimp and save our monthly salaries? Or why some people are more willing to spend gift money on a fancy dinner but hesitate to touch their savings for the same treat? The fascinating quirk of human behavior known as mental accounting is a concept popularized by Nobel laureate Richard Thaler in his seminal book, Nudge: Improving Decisions About Health, Wealth, and Happiness, co-authored with Cass R. Sunstein. Today, we'll delve into the intricacies of mental accounting and uncover how understanding this phenomenon can enhance your approach to mutual fund investments.
                        </span>
                    </div>
                    <div id='section2forleftsectionofmiddlesectionofblogsummary'>
                        <img src={confusedman} alt="" className='confusedmanimageforblogsummary' />
                    </div>
                    <div id='section3forleftsectionofmiddlesectionofblogsummary'>
                        <span id='headingforleftsectionofmiddlesectionofblogsummary'>
                            What is Mental Accounting?
                        </span>
                        <span id='textforleftsectionofmiddlesectionofblogsummary'>
                            Mental accounting is a psychological phenomenon where people categorize and treat money differently based on its origin, purpose, or intended use. Essentially, we create mental "accounts" for our finances, which influences how we allocate and spend money. This isn't always rational or optimal, but it’s a common way our brains simplify financial decisions.
                            <br /><br />
                            For example, you might have separate mental accounts for daily expenses, savings, and "fun money." Even though all money is fungible (one rupee is as good as another), we often treat these accounts as if they were distinct and non-interchangeable. This can lead to some puzzling financial behavior.
                            <br /><br />
                            In Nudge, Thaler and Sunstein illustrate mental accounting with the story of a man who keeps two bank accounts: one for daily expenses and one for savings. Despite having ample funds in his savings account, he refuses to dip into it for a short-term need, leading to unnecessary stress and financial juggling. This rigid compartmentalization is a classic example of mental accounting at work.
                        </span>
                    </div>
                    <div id='section3forleftsectionofmiddlesectionofblogsummary'>
                        <span id='headingforleftsectionofmiddlesectionofblogsummary'>
                            What is Mental Accounting?
                        </span>
                        <span id='textforleftsectionofmiddlesectionofblogsummary'>
                            Mental accounting is a psychological phenomenon where people categorize and treat money differently based on its origin, purpose, or intended use. Essentially, we create mental "accounts" for our finances, which influences how we allocate and spend money. This isn't always rational or optimal, but it’s a common way our brains simplify financial decisions.
                            <br /><br />
                            For example, you might have separate mental accounts for daily expenses, savings, and "fun money." Even though all money is fungible (one rupee is as good as another), we often treat these accounts as if they were distinct and non-interchangeable. This can lead to some puzzling financial behavior.
                            <br /><br />
                            In Nudge, Thaler and Sunstein illustrate mental accounting with the story of a man who keeps two bank accounts: one for daily expenses and one for savings. Despite having ample funds in his savings account, he refuses to dip into it for a short-term need, leading to unnecessary stress and financial juggling. This rigid compartmentalization is a classic example of mental accounting at work.
                        </span>
                    </div>
                    <div id='section3forleftsectionofmiddlesectionofblogsummary'>
                        <span id='headingforleftsectionofmiddlesectionofblogsummary'>
                            What is Mental Accounting?
                        </span>
                        <span id='textforleftsectionofmiddlesectionofblogsummary'>
                            Mental accounting is a psychological phenomenon where people categorize and treat money differently based on its origin, purpose, or intended use. Essentially, we create mental "accounts" for our finances, which influences how we allocate and spend money. This isn't always rational or optimal, but it’s a common way our brains simplify financial decisions.
                            <br /><br />
                            For example, you might have separate mental accounts for daily expenses, savings, and "fun money." Even though all money is fungible (one rupee is as good as another), we often treat these accounts as if they were distinct and non-interchangeable. This can lead to some puzzling financial behavior.
                            <br /><br />
                            In Nudge, Thaler and Sunstein illustrate mental accounting with the story of a man who keeps two bank accounts: one for daily expenses and one for savings. Despite having ample funds in his savings account, he refuses to dip into it for a short-term need, leading to unnecessary stress and financial juggling. This rigid compartmentalization is a classic example of mental accounting at work.
                        </span>
                    </div>
                    <div id='section3forleftsectionofmiddlesectionofblogsummary'>
                        <span id='headingforleftsectionofmiddlesectionofblogsummary'>
                            What is Mental Accounting?
                        </span>
                        <span id='textforleftsectionofmiddlesectionofblogsummary'>
                            Mental accounting is a psychological phenomenon where people categorize and treat money differently based on its origin, purpose, or intended use. Essentially, we create mental "accounts" for our finances, which influences how we allocate and spend money. This isn't always rational or optimal, but it’s a common way our brains simplify financial decisions.
                            <br /><br />
                            For example, you might have separate mental accounts for daily expenses, savings, and "fun money." Even though all money is fungible (one rupee is as good as another), we often treat these accounts as if they were distinct and non-interchangeable. This can lead to some puzzling financial behavior.
                            <br /><br />
                            In Nudge, Thaler and Sunstein illustrate mental accounting with the story of a man who keeps two bank accounts: one for daily expenses and one for savings. Despite having ample funds in his savings account, he refuses to dip into it for a short-term need, leading to unnecessary stress and financial juggling. This rigid compartmentalization is a classic example of mental accounting at work.
                        </span>
                    </div>
                    <span id='disclaimerforblogsummary'>
                        MUTUAL FUND INVESTMENTS ARE SUBJECT TO MARKET RISKS, READ ALL SCHEME RELATED DOCUMENTS CAREFULLY.
                    </span>
                </div>
                <div id='rightsectionofmiddlesectionofblogsummary'>
                    <div className='card1forrightsectionofmiddlesectionofblogsummary'>
                        <img src={blogimage} alt="" className='blogimage' />
                        <div id='rightsideofcard1forrightsectionofmiddlesectionofblogsummary'>
                            <div id='topofrightsideofcard1forrightsectionofmiddlesectionofblogsummary'>
                                <div id='leftsideoftopofrightsideofcard1forrightsectionofmiddlesectionofblogsummary'>
                                    <span className='spantext'>Jul 9, 2024</span>
                                    <svg xmlns="http://www.w3.org/2000/svg" width="2" height="16" viewBox="0 0 2 16" fill="none">
                                        <path d="M1 0V16" stroke="#E2E2E2" />
                                    </svg>
                                    <span className='spantext'>5 min read</span>
                                </div>
                                <div id='rightsideoftopofrightsideofcard1forrightsectionofmiddlesectionofblogsummary'>
                                    <img src={shareicon} alt="" className='shareicon' />
                                </div>
                            </div>
                            <div id='bottomofrightsideofcard1forrightsectionofmiddlesectionofblogsummary'>
                                Anchoring Bias Exposed: Your Money's Puppeteer.... <span className='goldenlink2'>READ MORE</span>
                            </div>
                        </div>
                    </div>
                    <div className='card1forrightsectionofmiddlesectionofblogsummary'>
                        <img src={blogimage} alt="" className='blogimage' />
                        <div id='rightsideofcard1forrightsectionofmiddlesectionofblogsummary'>
                            <div id='topofrightsideofcard1forrightsectionofmiddlesectionofblogsummary'>
                                <div id='leftsideoftopofrightsideofcard1forrightsectionofmiddlesectionofblogsummary'>
                                    <span className='spantext'>Jul 9, 2024</span>
                                    <svg xmlns="http://www.w3.org/2000/svg" width="2" height="16" viewBox="0 0 2 16" fill="none">
                                        <path d="M1 0V16" stroke="#E2E2E2" />
                                    </svg>
                                    <span className='spantext'>5 min read</span>
                                </div>
                                <div id='rightsideoftopofrightsideofcard1forrightsectionofmiddlesectionofblogsummary'>
                                    <img src={shareicon} alt="" className='shareicon' />
                                </div>
                            </div>
                            <div id='bottomofrightsideofcard1forrightsectionofmiddlesectionofblogsummary'>
                                Anchoring Bias Exposed: Your Money's Puppeteer.... <span className='goldenlink2'>READ MORE</span>
                            </div>
                        </div>
                    </div>
                    <div className='card1forrightsectionofmiddlesectionofblogsummary'>
                        <img src={blogimage} alt="" className='blogimage' />
                        <div id='rightsideofcard1forrightsectionofmiddlesectionofblogsummary'>
                            <div id='topofrightsideofcard1forrightsectionofmiddlesectionofblogsummary'>
                                <div id='leftsideoftopofrightsideofcard1forrightsectionofmiddlesectionofblogsummary'>
                                    <span className='spantext'>Jul 9, 2024</span>
                                    <svg xmlns="http://www.w3.org/2000/svg" width="2" height="16" viewBox="0 0 2 16" fill="none">
                                        <path d="M1 0V16" stroke="#E2E2E2" />
                                    </svg>
                                    <span className='spantext'>5 min read</span>
                                </div>
                                <div id='rightsideoftopofrightsideofcard1forrightsectionofmiddlesectionofblogsummary'>
                                    <img src={shareicon} alt="" className='shareicon' />
                                </div>
                            </div>
                            <div id='bottomofrightsideofcard1forrightsectionofmiddlesectionofblogsummary'>
                                Anchoring Bias Exposed: Your Money's Puppeteer.... <span className='goldenlink2'>READ MORE</span>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
            <div id='ratingdivforblogsummary'>
                <span id='headingforratingdivforblogsummary'>
                    Did you find the article interesting?
                </span>
                <div id='divforratingsectionforblogsummary'>
                    <div className='ratingdiv'>
                        <img src={image1} alt="" className='ratingimage' />
                        <span className='textforratingimage'>1</span>
                    </div>
                    <div className='ratingdiv'>
                        <img src={image2} alt="" className='ratingimage' />
                        <span className='textforratingimage'>2</span>
                    </div>
                    <div className='ratingdiv'>
                        <img src={image3} alt="" className='ratingimage' />
                        <span className='textforratingimage'>3</span>
                    </div>
                    <div className='ratingdiv'>
                        <img src={image4} alt="" className='ratingimage' />
                        <span className='textforratingimage'>4</span>
                    </div>
                    <div className='ratingdiv'>
                        <img src={image5} alt="" className='ratingimage' />
                        <span className='textforratingimage'>5</span>
                    </div>
                </div>
            </div>

            <div id='mainframe8' style={{ marginTop: '128px' }}>
                <div id='insideframeofmainframe8'>
                    <img src={logo} alt="" />
                    <div id='contentinsideframeofmainframe8'>
                        <div id='insidecontentinsideframeofmainframe8'>
                            <span id='titleofinsidecontentinsideframeofmainframe8'>
                                QUICK LINK
                            </span>
                            <div className='linksofinsidecontentinsideframeofmainframe8'>
                                
                                <div className='linkseparator'>
                                    <span className='linktitle2'>Our Funds</span>
                                    <span className='goldenlink'>Coming soon...</span>
                                </div>
                                <div className='linkseparator'>
                                    <span className='linktitle2'>Tools & Resources</span>
                                    <span className='goldenlink'>Coming soon...</span>
                                </div>
                            </div>
                        </div>
                        <div id='insidecontentinsideframeofmainframe8'>
                            <span id='titleofinsidecontentinsideframeofmainframe8'>
                                SEBI
                            </span>
                            <div className='linksofinsidecontentinsideframeofmainframe8'>
                                <span className='linktitle2'>SEBU Smart ODR</span>
                                <span className='linktitle2'>SEBI Scores</span>
                                <span className='linktitle2'>SEBU Scores App (ios)</span>
                                <span className='linktitle2'>SEBU Scores App (Android)</span>

                            </div>
                        </div>
                        <div id='divforconnect'>
                            <span id='titleofdivforconnect'>CONNECT WITH US</span>
                            <div id='divinsideconnect'>
                                <div className='socialmediaicondiv'>
                                    <img src={facebook} alt="" className='socialmediaicon' />
                                </div>
                                <div className='socialmediaicondiv'>
                                    <img src={twitter} alt="" className='socialmediaicon' />
                                </div>
                                <div className='socialmediaicondiv'>
                                    <img src={linkedin} alt="" className='socialmediaicon' />
                                </div>
                                <div className='socialmediaicondiv'>
                                    <img src={insta} alt="" className='socialmediaicon' />
                                </div>
                                <div className='socialmediaicondiv'>
                                    <img src={youtube} alt="" className='socialmediaicon' />
                                </div>
                            </div>
                        </div>
                    </div>
                </div>
                <span id='copyrighttext'>
                    Copyright © 2025 Wealth Company Private Limited. | All rights reserved.
                </span>
            </div>
        </>
    )
}

export default Blogsummary
